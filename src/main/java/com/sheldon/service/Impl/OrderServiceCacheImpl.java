...
public class OrderServiceCacheImpl implements OrderServiceCache {
  ...
  public Boolean addOrder(Order order) {
    ...
    String VipSeatNo = NumGen.getVipSeat();
    String NormalSeatNo = NumGen.getNormalSeat();
    String GateNo = NumGen.getGate();
    tid = ticketId;
    order.setId(ticketId);
    order.setTicketNo(ticketNo);
    if (order.getClassType().equals("头等舱")) {
      order.setSeatNo(VipSeatNo);
    }
    if (order.getClassType().equals("经济舱")){
      order.setSeatNo(NormalSeatNo);
    }
    order.setGate(GateNo);
    order.setIsPaid("否");
  ...
  public List<Order> getQuery(Order order) {
    ...
    lqwQ.eq(Strings.isNotEmpty(order.getCitizenId()), Order::getCitizenId, order.getCitizenId());
