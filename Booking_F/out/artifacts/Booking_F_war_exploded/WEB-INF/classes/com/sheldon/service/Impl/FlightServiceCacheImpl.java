...
public class FlightServiceCacheImpl implements FlightServiceCache {
  ...
  @Override
  public List<Flight> getByFlightId(String flightId) {
    return flightDao.getByFlightId(flightId);
  }

  @Override
  public Flight getByUuid(String uuid) {
    return flightDao.getByUuid(uuid);
  }
  ...
  @Override
  public List<Flight> getQuery(Flight flight) {
    LambdaQueryWrapper<Flight> lqwQ = new LambdaQueryWrapper<Flight>();
    lqwQ.like(Strings.isNotEmpty(flight.getFromCity()), Flight::getFromCity, flight.getFromCity());
    lqwQ.like(Strings.isNotEmpty(flight.getToCity()), Flight::getToCity, flight.getToCity());
    lqwQ.like(Strings.isNotEmpty(flight.getFromTime()), Flight::getFromTime, flight.getFromTime());
    return flightDao.selectList(lqwQ);
  }
